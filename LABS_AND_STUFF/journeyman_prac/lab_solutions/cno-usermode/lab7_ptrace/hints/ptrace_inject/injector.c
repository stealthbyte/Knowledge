...
int main(int argc, char* argv[]) {
	...
	reqregs.rdi = 0;
	reqregs.rsi = STACKSIZE + pathareasz;
	reqregs.rdx = PROT_READ | PROT_WRITE;
	reqregs.r10 = MAP_GROWSDOWN | MAP_ANONYMOUS | MAP_PRIVATE;
	reqregs.r8  = -1;
	reqregs.r9  = 0;
	...
	reqregs = savedregs;
	reqregs.rip = (uint64_t)targ_libc_entry;
	reqregs.rsp = mappedloc + (STACKSIZE - 0x80);
	reqregs.rax = (uint64_t)targ_dlopen;
	//__libc_dlopen_mode(path<rdi>, flags<rsi>)
	reqregs.rdi = mappedloc + STACKSIZE;
